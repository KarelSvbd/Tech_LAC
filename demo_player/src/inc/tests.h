/* # $Id: tests.h 10968 2021-02-02 21:35:54Z marechal $ */
/* C. Marechal, 02/2010
Some changes in order to avoid Splint warnings. Look for cm pattern. 
...
#define INFO_LENGTH_MAX 80 /* cm : in order to use snprintf() instead of sprintf() */
/* --- Main test framework --- 
...
#define START_TESTS() \
	int main(void) { \
		static int fails = 0; \
		static int tests = 0; \
		static int testfails = 0; \
		static int passes = 0; \
		static /*@observer@*/ char *name; /* cm */ \
		static /*@observer@*/ char *reason = NULL; /* cm */ \
		static char info[INFO_LENGTH_MAX]; \
		static double epsilon; \
		/*@unused@*/ static int continueTests = 1; /* cm */\
		SET_EPSILON_DEFAULT(); \
 		(void)snprintf(info, INFO_LENGTH_MAX, " ");
...
#define gcc_NE(a, b) ASSERT_NOT_EQUALS(a, b);
...
#define FLOAT_EQUALS(a, b) /*@i@*/(ABS((float)(a) - (float)(b)) < epsilon /* cm : in order to avoid float comparison splint warning */\
